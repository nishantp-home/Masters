# Principle component analysis (PCA) with Neural Network (NNet)
# This file is same as classification.Rmd
# Only change in model_nnet(...method = "pcaNNet",...)
...
```{r}
cm_nnet <- confusionMatrix(pred_nnet, testing$default)
```
```{r}
cm_nnet
```
